...
class BrowserHost {
  ...
  // The embedded sign-in starts on its own whenever the browser is signed out, so a passkey-only
  // account always finds one in flight. Returning that promise made Passkey sign in wait for the
  // embedded login's own timeout instead of ever opening Chrome, so an explicit passkey request
  // supersedes an embedded login rather than joining it.
  openPasskeyLogin() {
